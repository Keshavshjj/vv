...
 Usage: /remove <userid>'''
        ...
        response = "Purchase karle bsdk:- @GyramgeYT ."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"CHUDAI start : {target}:{port} for {time}\nSEC Jabtak YE PORT run krrha hai to iske bichme koi PORT nhi Dalna Bhenchod"
    bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "Usage :- /sharp <target> <port> <time> "  # Updated command syntax
    else:
...
def welcome_start(message):
    ...
    response = f'''Gyrange ke LODE pe aapka swagat hai, {user_name}! Sabse acche se bgmi ki maa behen yahi hack karta hai. Kharidne ke liye Gyrange se sampark karein.
